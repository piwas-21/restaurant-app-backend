...
public class RestaurantInfoMutationTests : IntegrationTestBase
{
    ...
    [Fact]
    public async Task Update_AsAdmin_RoundTripsEntrancePosition()
    {
        AuthenticateAsAdmin();

        // Set the entrance marker's percent coordinates; they must persist and
        // surface on the public GET, where the reservation floor plan reads them.
        var response = await Client.PutAsJsonAsync(
            "/api/restaurant-info",
            new UpdateRestaurantInfoCommand(
                "Rumi", "Rue du Grand-Pré 99", null, "Genève", "1202", "Switzerland",
                null, null, "contact@rumirestaurant.ch", null,
                EntrancePositionX: 12.5m,
                EntrancePositionY: 87.25m));

        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var dto = await ReadDataAsync<RestaurantInfoDto>(response);
        dto.EntrancePositionX.Should().Be(12.5m);
        dto.EntrancePositionY.Should().Be(87.25m);

        var getResp = await Client.GetAsync("/api/restaurant-info");
        var info = await ReadDataAsync<RestaurantInfoDto>(getResp);
        info.EntrancePositionX.Should().Be(12.5m);
        info.EntrancePositionY.Should().Be(87.25m);

        // Full-replace semantics: a PUT that omits the entrance position
        // clears it back to null (frontend default position).
        ...
        var cleared = await ReadDataAsync<RestaurantInfoDto>(clearResp);
        cleared.EntrancePositionX.Should().BeNull();
        cleared.EntrancePositionY.Should().BeNull();
    }

    // Boundary values 0 and 100 are valid (InclusiveBetween); anything
    // outside — including fractionally outside — is a 400.
    [Theory]
    [InlineData(-1, 50)]      // X below range
    [InlineData(101, 50)]     // X above range
    [InlineData(50, -1)]      // Y below range
    [InlineData(50, 101)]     // Y above range
    [InlineData(-0.01, 100)]  // X fractionally below
    [InlineData(0, 100.01)]   // Y fractionally above
    public async Task Update_AsAdmin_EntrancePositionOutOfRange_Returns400(
        double x, double y)
    {
        AuthenticateAsAdmin();
        ...
        var response = await Client.PutAsJsonAsync(
            ...
            new UpdateRestaurantInfoCommand(
                "Rumi", "Rue du Grand-Pré 99", null, "Genève", "1202", "Switzerland",
                null, null, "contact@rumirestaurant.ch", null,
                EntrancePositionX: (decimal)x,
                EntrancePositionY: (decimal)y));
    ...
    [InlineData(0, 0)]
    [InlineData(100, 100)]
    public async Task Update_AsAdmin_EntrancePositionBoundary_Accepted(double x, double y)
    {
        AuthenticateAsAdmin();
        ...
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var dto = await ReadDataAsync<RestaurantInfoDto>(response);
        dto.EntrancePositionX.Should().Be((decimal)x);
        dto.EntrancePositionY.Should().Be((decimal)y);
    }
