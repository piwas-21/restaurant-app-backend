...
if [[ -x "$REVIEW_GATE_INSTALL" ]]; then
  if bash "$REVIEW_GATE_INSTALL" backend >/dev/null 2>&1; then
    ok "RUMI review-gate pre-push hook restored (chains pre-commit's pre-push checks)."
  else
    warn "review-gate hook reinstall failed — run scripts/review-gate/install.sh backend from the workspace root."
  fi
