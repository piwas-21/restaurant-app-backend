...
public static class ErrorCodes
{
    ...
    /// <summary>
    /// Returned with a 404 when the BASKET ROW itself is gone — the cleanup service reaped it, or
    /// the session id expired — so the request never got as far as looking at an item.
    /// This is a real failure the guest must be told about: the client cannot silently resync,
    /// because <c>GetBasketQuery</c> answers a missing basket with an empty basket and a SUCCESS,
    /// so resyncing here replaces the whole cart with "Your cart is empty".
    /// </summary>
    public const string BasketNotFound = "BasketNotFound";

    /// <summary>
    /// Returned with a 404 when the basket exists but the addressed ITEM does not — normally
    /// because the guest already removed it in another tab. This is the one basket 404 a client may
    /// treat as benign and recover from by resyncing.
